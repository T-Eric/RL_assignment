...
class Policy(nn.Module):
    def __init__(self, obs_dim=4, latent_dim=0, action_dim=2, action_limit=(2.0, 2.0)):
        ...
        # shared encoder
        self.sensor_branch = SensorBranch(input_dim=obs_dim, hidden_dim=HIDDEN_DIM)

        # actor conditional path
        if latent_dim > 0:
            self.latent_encoder = LatentEncoder(latent_dim=latent_dim, hidden_dim=LATENT_HIDDEN_DIM)
    ...
    def act(self, obs, deterministic=False):
        ...
        action_scaled = torch.tanh(action_pre_tanh) * self.action_scale.to(action_pre_tanh.device)
