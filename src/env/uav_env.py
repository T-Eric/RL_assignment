...
class UAVNavEnv(gym.Env):
    ...
    def __init__(self, pointcloud_path, env_params=None, save_dir=None,
        ...
        self.observation_shape = {"sensor": (self.num_dirs+4+self.latent_dim,)}
        self.action_shape = (2,)
        ...
        # per-initial historical coverage
        self.coverage_maps={
    ...
    def reset(self, initial_pose, target_center, initial_id=None, latent=None):
        ...
        self._prev_target_dist = np.linalg.norm(sensor[self.num_dirs+2:self.num_dirs+4])
        return obs
    ...
    def step(self, action):
        ...
        if self.use_soft_collision and obstacle_dist < self.safe_distance:
            reward -= self.safe_penalty_coef * (self.safe_distance - obstacle_dist) ** 2
        ...
        if self.use_history_vis and self.curr_initial_id is not None:
            ...
            count=coverage_map.get(curr_cell, 0)
            history_reward+=self.history_bonus_coef/np.sqrt(1.0+count)
            # coverage_map[curr_cell]=count+1
    ...
    def _get_obs(self):
        ...
        # directional distances, dim=num_dirs
